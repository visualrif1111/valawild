...
// 2400x1500 viewBox → 1.6:1 landscape ratio
const sceneFill = "relative flex-shrink-0 w-[max(100vw,160vh)] h-[max(100vh,62.5vw)]";
...
export default function KilimanjaroBackground() {
  ...
  // ── Atmospheric scene transition overlay ─────────────────────────────────
  ...
  // ── MOUBNT mountain SVG transforms ───────────────────────────────────────
  // Vertical ascent: mountain drifts upward as you scroll (reveal body from peak)
  const moubntScrollY = useTransform(scrollYProgress, [0, 1], ["0vh", "-100vh"]);
  // Opacity shifts 15% → 65% → 15% across the journey
  const moubntOpacity = useTransform(
    scrollYProgress,
    [0,   0.08, 0.25, 0.50, 0.72, 0.90, 1],
    [0.15, 0.55, 0.65, 0.60, 0.55, 0.35, 0.18]
  );
  // Subtle scale expansion as you ascend
  const moubntScale = useTransform(scrollYProgress, [0, 0.5, 1], [1.0, 1.04, 1.08]);
  ...
  return (
    <div className="relative w-full h-[3600vh]" style={{ backgroundColor: BASE_COLOR }}>
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        ...
        {/* ── Layer 0: MOUBNT mountain SVG — the central environmental element ── */}
        <motion.div
          style={{ y: moubntScrollY, x: parallaxX_bg, opacity: moubntOpacity, scale: moubntScale }}
          className="absolute inset-0 will-change-transform"
        >
          {/* Float wrapper: slow ambient vertical breathing */}
          <motion.div
            animate={{ y: [0, -16, 0] }}
          ...
          >
            <img
              ...
              style={{
                ...
                height: 'auto',
                display: 'block',
                // Fade absolute top (sky fades in), keep mountain body solid, fade out at bottom
                maskImage:
                  'linear-gradient(to bottom, rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.7) 6%, black 14%, black 60%, rgba(0,0,0,0.6) 75%, transparent 100%)',
                WebkitMaskImage:
                  'linear-gradient(to bottom, rgba(0,0,0,0.15) 0%, rgba(0,0,0,0.7) 6%, black 14%, black 60%, rgba(0,0,0,0.6) 75%, transparent 100%)',
              }}
            />

            {/* Ice-blue atmospheric tint over the SVG — enhances the Kilimanjaro glacier palette */}
            <div
              className="absolute inset-0 pointer-events-none mix-blend-screen"
              style={{
                background:
                  'linear-gradient(to bottom, rgba(130,170,210,0.10) 0%, rgba(160,200,230,0.06) 40%, rgba(100,150,200,0.08) 100%)',
              }}
            />
          </motion.div>
        </motion.div>

        {/* ── Layer 0b: Ambient atmospheric radial glow around mountain ─── */}
        <motion.div
          style={{ x: parallaxX_bg, y: parallaxY_bg }}
          className="absolute inset-0 pointer-events-none"
        >
          {/* Deep navy vignette — frames the mountain */}
          <div
            className="absolute inset-0"
            style={{
              background:
                'radial-gradient(ellipse 70% 80% at 50% 30%, transparent 30%, rgba(5,8,16,0.55) 100%)',
            }}
          />
          {/* Glacier white ambient centre glow */}
          <motion.div
            ...
            style={{
              ...
              height: '45vh',
              background:
                'radial-gradient(ellipse 100% 100% at 50% 20%, rgba(210,230,248,0.18) 0%, transparent 70%)',
              filter: 'blur(30px)',
            }}
          />
        </motion.div>

        {/* ── Layer 1: KiliScenes — scroll narrative on top of mountain ─── */}
        <motion.div style={{ y: parallaxY_bg }} className="absolute inset-0">
          <motion.div style={{ y: yStack, x: parallaxX_bg }} className="absolute top-0 left-0 w-full h-[900vh] will-change-transform">
            ...
            <motion.div style={{ display: displayBg1 }} className={`${sceneClass} top-0`}>
              ...
              <div className="absolute bottom-0 w-full h-[50vh] z-10" style={{ background: `linear-gradient(to top, ${BASE_COLOR}, transparent)` }} />
            ...
            <motion.div style={{ display: displayBg2 }} className={`${sceneClass} top-[100vh]`}>
              ...
              <div className="absolute top-0 w-full h-[50vh] z-10" style={{ background: `linear-gradient(to bottom, ${BASE_COLOR}, transparent)` }} />
              <div className="absolute bottom-0 w-full h-[50vh] z-10" style={{ background: `linear-gradient(to top, ${BASE_COLOR}, transparent)` }} />
              <div className="absolute inset-0 shadow-[inset_0_0_150px_rgba(0,0,0,0.25)] pointer-events-none z-10" />
            </motion.div>
        ...
        {/* ── Drifting translucent cloud / mist ribbon overlays ─────────── */}
        <motion.div style={{ x: parallaxX_fg2, y: parallaxY_fg2 }} className="absolute inset-0 pointer-events-none z-30">

          {/* Ribbon 1 — wide slow band, mist silver */}
          <motion.div
            ...
            style={{
              ...
              height: '6vh',
              background:
                'linear-gradient(to right, transparent 0%, rgba(185,205,225,0.09) 20%, rgba(200,218,238,0.13) 50%, rgba(185,205,225,0.09) 80%, transparent 100%)',
              filter: 'blur(18px)',
            }}
          />

          {/* Ribbon 2 — narrower, ice blue, offset timing */}
          <motion.div
            animate={{ x: ["10%", "-10%", "10%"], y: [0, 8, 0] }}
            transition={{ duration: 55, repeat: Infinity, ease: "easeInOut", delay: 8 }}
            className="absolute"
            style={{
              top: '38%',
              left: '-15%',
              ...
              height: '4vh',
              background:
                'linear-gradient(to right, transparent 0%, rgba(170,200,230,0.07) 25%, rgba(195,215,240,0.10) 55%, rgba(170,200,230,0.07) 80%, transparent 100%)',
              filter: 'blur(24px)',
            }}
          />

          {/* Ribbon 3 — mid-height, slightly opaque glacier white */}
          <motion.div
            animate={{ x: ["-12%", "5%", "-12%"], y: [0, -8, 0] }}
            transition={{ duration: 60, repeat: Infinity, ease: "easeInOut", delay: 18 }}
            className="absolute"
            style={{
              top: '55%',
              left: '-8%',
              ...
              height: '5vh',
              background:
                'linear-gradient(to right, transparent 0%, rgba(210,225,240,0.06) 30%, rgba(220,235,248,0.10) 60%, rgba(210,225,240,0.06) 85%, transparent 100%)',
              filter: 'blur(20px)',
            }}
          />

          {/* Ribbon 4 — low horizon, champagne-tinted, subtle */}
          <motion.div
            animate={{ x: ["5%", "-6%", "5%"], y: [0, 10, 0] }}
            transition={{ duration: 70, repeat: Infinity, ease: "easeInOut", delay: 30 }}
            className="absolute"
            style={{
              top: '72%',
              left: '-12%',
              ...
              height: '3.5vh',
              background:
                'linear-gradient(to right, transparent 0%, rgba(215,210,200,0.05) 35%, rgba(225,218,205,0.08) 55%, rgba(215,210,200,0.05) 80%, transparent 100%)',
              filter: 'blur(16px)',
            }}
          />
        </motion.div>

        {/* ── Floating organic cloud shapes ─────────────────────────────── */}
        ...
        {/* ── Snow/ice particles + cold light beam ──────────────────────── */}
        <motion.div style={{ x: parallaxX_fg1, y: parallaxY_fg1 }} className="absolute inset-0 pointer-events-none mix-blend-screen">
          ...
          {/* Cold light beam angled from upper right */}
          <motion.div
          ...
          />
          {/* Secondary diagonal beam — ice blue */}
          <motion.div
            className="absolute top-[-10%] left-[30%] w-[30vw] h-[120vh] bg-gradient-to-b from-[#B8D4EC]/3 to-transparent rotate-[-15deg] blur-2xl"
